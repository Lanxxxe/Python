# <!DOCTYPE SHORTEST REMAINING TIME FIRST>
# needed to add: exception catchers (maybe)



def algo_srtf(processes):
    processlist = {}
        ...
        for key, value in arrival_times.items():
            if value == process_time:
                if current_process == "IDLE":
                    ...
                    waitlist.append(key)
                    waitlist= sorted(waitlist, key=lambda value: burst_times[value])
        if len(waitlist) > 0 and burst_times[current_process] > 0:
            if burst_times[current_process] > burst_times[waitlist[0]]:
...
gantt_chart, waiting_times, turnaround_times  = algo_srtf(processlist)
